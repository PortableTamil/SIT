...
//     return : 
//-----------------------------------------------------------------------------
CToolbar::CToolbar(HINSTANCE hInstance,HWND hwndParent,int ControlId)
{
    this->Constructor(hInstance,hwndParent,FALSE,FALSE,0,0,ControlId);
}
...
//     return : 
//-----------------------------------------------------------------------------
CToolbar::CToolbar(HINSTANCE hInstance,HWND hwndParent,BOOL bFlat,BOOL bListStyle,int ControlId)
{
    this->Constructor(hInstance,hwndParent,bFlat,bListStyle,0,0,ControlId);
}
...
//     return : 
//-----------------------------------------------------------------------------
CToolbar::CToolbar(HINSTANCE hInstance,HWND hwndParent,BOOL bFlat,BOOL bListStyle,DWORD dwIconWidth,DWORD dwIconHeight,int ControlId)
{
    this->Constructor(hInstance,hwndParent,bFlat,bListStyle,dwIconWidth,dwIconHeight,ControlId);
}
//-----------------------------------------------------------------------------
// Name: Constructor
// Object: constructor
...
//     return : 
//-----------------------------------------------------------------------------
void CToolbar::Constructor(HINSTANCE hInstance,HWND hwndParent,BOOL bFlat,BOOL bListStyle,DWORD dwIconWidth,DWORD dwIconHeight,int ControlId)
{
    this->hInstance=hInstance;
    this->pToolbarEvents = NULL;
    ...
    DWORD dwStyle=WS_CHILD | TBSTYLE_TOOLTIPS | WS_VISIBLE|WS_GROUP;
                    // TBSTYLE_TRANSPARENT
...
BOOL CToolbar::SetEventsHandler(IToolbarEvents* pToolbarEvents)
{
    this->pToolbarEvents = pToolbarEvents;
    return TRUE;
}

IToolbarEvents* CToolbar::GetEventsHandler()
{
    return this->pToolbarEvents;
}
...
BOOL CToolbar::SetPosition(tagCToolbarPosition Position)
{
    ...
    Style&=~(CCS_TOP | CCS_LEFT | CCS_BOTTOM | CCS_RIGHT|CCS_NORESIZE|CCS_NOPARENTALIGN);
    // Add new position style
    Style|=(LONG_PTR)Position;

    if (Position == ToolbarPosition_USER)
        Style|=CCS_NORESIZE|CCS_NOPARENTALIGN;

    // apply new style
    bRet= (SetWindowLongPtr(this->hwndTB,GWL_STYLE,Style) !=0);
...
//     return : 
//-----------------------------------------------------------------------------
BOOL CToolbar::SetPosition(LONG x,LONG y,LONG Width,LONG Height)
{
    BOOL bRet = this->SetPosition(ToolbarPosition_USER);

    SetWindowPos(this->hwndTB,HWND_NOTOPMOST,
                x,
                y,
                Width,
                Height,
...
BOOL CToolbar::SetDirection(BOOL bHorizontal)
{
    BOOL bRet;
    LONG_PTR Style;
    this->VerticalDirection = !bHorizontal;
    Style=GetWindowLongPtr(this->hwndTB,GWL_STYLE);
    if (this->VerticalDirection)
        Style|=CCS_VERT|TBSTYLE_WRAPABLE|TBSTYLE_TRANSPARENT;
    else
        Style&=~CCS_VERT;
    ...
    // for each button add TBSTATE_WRAP style
    int NbItems = this->GetButtonCount();
    int Cnt;
    int ButtonId;
    BYTE ButtonState;
    // msdn: The key to creating a vertical toolbar is to include CCS_VERT in the window style, and to set the TBSTATE_WRAP style 
    for (Cnt=0;Cnt<NbItems;Cnt++)
    {
        ButtonId = this->GetButtonId(Cnt);
        ButtonState = this->GetButtonState(ButtonId);
        if (this->VerticalDirection)
            ButtonState|=TBSTATE_WRAP;
        else
            ButtonState&=~TBSTATE_WRAP;
        this->SetButtonState(ButtonId,ButtonState);
    }
...
void CToolbar::AddSeparator()
{
    ...
    Button.iBitmap = I_IMAGENONE; // msdn : If the button is a separator, that is, if fsStyle is set to BTNS_SEP, iBitmap determines the width of the separator, in pixels
    Button.idCommand = 0;
    Button.fsState = TBSTATE_ENABLED;
    Button.fsStyle = BTNS_SEP; 
    if (this->VerticalDirection)
    {
        // Fix me : vertical splitter is shown and user have to set specific width to show full button and hide vertical splitter
        // example : for 16*16 items : vertical toolbar width of 28 show full button and hide vertical splitter
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIcon,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,IdIcon,IdIcon,ToolTip,Width,Height);
}
//-----------------------------------------------------------------------------
// Name: AddButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIcon,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,BTNS_SHOWTEXT|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIcon,IdIcon,IdIcon,NULL, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddCheckButton(int ButtonID,int IdIcon,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddCheckButton(ButtonID,IdIcon,IdIcon,IdIcon,ToolTip, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddCheckButton(int ButtonID,int IdIcon,int IdIconDisabled,int IdIconHot,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,NULL,BTNS_CHECK|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIcon,IdIconDisabled,IdIconHot,ToolTip, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,int IdIcon,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,NULL,BTNS_BUTTON|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIcon,IdIcon,IdIcon,ToolTip, Width, Height);
}
//-----------------------------------------------------------------------------
// Name: AddButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,int IdIconEnable,int IdIconDisable,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,NULL,BTNS_BUTTON|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconEnable,ToolTip, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,NULL,BTNS_BUTTON|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconHot,ToolTip, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIconEnable,int IdIconDisable,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,BTNS_SHOWTEXT|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconEnable,NULL, Width, Height);
}
//-----------------------------------------------------------------------------
// Name: AddButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIconEnable,int IdIconDisable,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,BTNS_SHOWTEXT|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconEnable,ToolTip, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIconEnable,int IdIconDisable,int IdIconHot,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,BTNS_SHOWTEXT|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconHot,NULL, Width, Height);
}
//-----------------------------------------------------------------------------
// Name: AddButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,BTNS_SHOWTEXT|BTNS_AUTOSIZE ,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconHot,ToolTip, Width, Height);
}
//-----------------------------------------------------------------------------
// Name: AddButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,BYTE ButtonStyle,BYTE ButtonState,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,int Width,int Height)
{
    return this->AddButton(ButtonID,ButtonText,ButtonStyle,ButtonState,IdIconEnable,IdIconDisable,IdIconHot,ToolTip,NULL, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddButton(int ButtonID,TCHAR* ButtonText,BYTE ButtonStyle,BYTE ButtonState,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,int Width,int Height)
{
    TBBUTTON Button={0};
    int iIconEnable;
    int iIconDisable;
    ...
    // load enable icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdIconEnable),IMAGE_ICON, Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
        return FALSE;
    ...
    // load disable icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdIconDisable),IMAGE_ICON,Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
    {
    ...
    // load hot icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdIconHot),IMAGE_ICON,Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
    {
...
//          TCHAR* ToolTip : button tooltip.
//          CPopUpMenu* PopUpMenu : Popup menu (only for drop down buttons)
//          BOOL bWholeDropDown : TRUE for full drop down (arrow not separated)
//     out :
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddDropDownButton(int ButtonID,int IdIcon,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,BOOL bWholeDropDown,int Width,int Height)
{
    return this->AddDropDownButton(ButtonID,NULL,IdIcon,ToolTip,PopUpMenu,bWholeDropDown, Width, Height);
}

BOOL  CToolbar::AddDropDownButton(int ButtonID,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,BOOL bWholeDropDown,int Width,int Height)
{
    return this->AddDropDownButton(ButtonID,NULL,IdIconEnable,IdIconDisable,IdIconHot,ToolTip,PopUpMenu,bWholeDropDown, Width, Height);
}
//-----------------------------------------------------------------------------
// Name: AddDropDownButton
// Object: add button to toolbar
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddDropDownButton(int ButtonID,TCHAR* ButtonText,int IdIcon,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,BOOL bWholeDropDown,int Width,int Height)
{
    return this->AddDropDownButton(ButtonID,ButtonText,TBSTATE_ENABLED,IdIcon,IdIcon,IdIcon,ToolTip,PopUpMenu,bWholeDropDown, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddDropDownButton(int ButtonID,TCHAR* ButtonText,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,BOOL bWholeDropDown,int Width,int Height)
{
    return this->AddDropDownButton(ButtonID,ButtonText,TBSTATE_ENABLED,IdIconEnable,IdIconDisable,IdIconHot,ToolTip,PopUpMenu,bWholeDropDown, Width, Height);
}
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::AddDropDownButton(int ButtonID,TCHAR* ButtonText,BYTE ButtonState,int IdIconEnable,int IdIconDisable,int IdIconHot,TCHAR* ToolTip,CPopUpMenu* PopUpMenu,BOOL bWholeDropDown,int Width,int Height)
{
    BYTE ButtonStyle;
    if (bWholeDropDown)
    ...
    return this->AddButton(ButtonID,ButtonText,ButtonStyle,ButtonState,IdIconEnable,IdIconDisable,IdIconHot,ToolTip,PopUpMenu, Width, Height);
}
...
CPopUpMenu* CToolbar::GetButtonDropDownMenu(int ButtonID)
{
    ...
    CPopUpMenu* pPopUpMenu = ((CToolbarButtonUserData*)ButtonInfo.lParam)->PopUpMenu;

    // calling OnToolbarDropDownMenuBeforeMenuShow here allow it to be called for rebar chevron too
    if (this->pToolbarEvents)
        this->pToolbarEvents->OnToolbarDropDownMenuBeforeMenuShow(this,pPopUpMenu);
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::ReplaceIcon(int ButtonID,CToolbar::ImageListType ImgListType,int IdNewIcon,int Width,int Height)
{
    HIMAGELIST hImgL;
    HICON hicon;
    ...
    // load icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdNewIcon),IMAGE_ICON,Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
        return FALSE;
...
//     return : TRUE on success, FALSE on error
//-----------------------------------------------------------------------------
BOOL CToolbar::ReplaceAllIcons(int ButtonID,int IdIcon,int IdIconDisabled,int IdIconHot,int Width,int Height)
{
    HIMAGELIST hImgL;
    HICON hicon;
    ...
    // load icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
        return FALSE;
    ...
    // load icon
    hicon=(HICON)LoadImage(this->hInstance, MAKEINTRESOURCE(IdIconDisabled),IMAGE_ICON,Width, Height,LR_DEFAULTCOLOR|LR_SHARED);
    if (!hicon)
        return FALSE;
...
// Parameters :
//     in  : int ButtonID : button index in toolbar
//           BYTE State : button state (TBSTATE_CHECKED , TBSTATE_ELLIPSES ,TBSTATE_ENABLED ,
...
BOOL CToolbar::SetButtonCheckedState(int ButtonID,BOOL bChecked)
{
    TBBUTTONINFO  ButtonInfo={0};
    // retrieve button param
    ButtonInfo.dwMask=TBIF_STATE;
...
// Parameters :
//     in  : int ButtonID : button id
//           BYTE Style : new style to apply
//     out :
//     return : Button Style TBSTYLE_CHECK, TBSTYLE_DROPDOWN, ...
//-----------------------------------------------------------------------------
BYTE CToolbar::SetButtonStyle(int ButtonID,BYTE Style)
{
    TBBUTTONINFO  ButtonInfo={0};
    // retrieve button param
    ButtonInfo.dwMask=TBIF_STYLE;
        ...
        case TBN_DROPDOWN:
                {
                    ...
                    pPopUpMenu=this->GetButtonDropDownMenu((int)idButton);
                    if (!pPopUpMenu)
                        return TRUE;
                    ...
                    // show menu                   
                    UINT MenuId;
                    MenuId=pPopUpMenu->Show(rc.left, rc.bottom, this->hwndTB);
                    if (MenuId)// if a menu has been selected
                    {
                        if (this->pToolbarEvents)
                            this->pToolbarEvents->OnToolbarDropDownMenu(this,pPopUpMenu,MenuId);
                    }
                }
                break;
        default:
