...
CRebar::CRebar(HINSTANCE hInstance,HWND hwndParent,int DialogControlId)
{
    this->pRebarEvents=NULL;
    ...
    this->hwndRebar = CreateWindowEx(WS_EX_TOOLWINDOW,
                                    REBARCLASSNAME,
                                    NULL,
                                    WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN | WS_GROUP |
                                    WS_CLIPSIBLINGS | RBS_VARHEIGHT | // RBS_DBLCLKTOGGLE |
                                    CCS_NODIVIDER,
                                    0,0,0,0,
                                    hwndParent,
                                    (HMENU)DialogControlId,// hope to be unique
                                    hInstance,
                                    NULL);
...
BOOL CRebar::SetEventsHandler(IRebarEvents* pRebarEvents)
{
    this->pRebarEvents = pRebarEvents;
    return TRUE;
}
...
void CRebar::Invalidate()
{
    if (this->hwndRebar)
        ::InvalidateRect(this->hwndRebar,NULL,FALSE);
}

void CRebar::Redraw()
{
    if (this->hwndRebar)
        ::RedrawWindow(this->hwndRebar,NULL,NULL,RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW);
}
...
UINT CRebar::AddToolBarBand(HWND hwndToolBar,TCHAR* Name,BOOL NewLine,BOOL FixedSize,BOOL NoGripper)
{
    ...
    {
        // RBBS_GRIPPERALWAYS must be set else first bar positioning and cxIdeal will be outdated when inserting second bar
        rbBand.fStyle|=RBBS_GRIPPERALWAYS;
    }

    // Get the height of the toolbar.
    DWORD BtnSize = (DWORD)SendMessage(hwndToolBar, TB_GETBUTTONSIZE, 0,0);
    RECT RectButton={0,0,16,16}; // default button size in case of no button
    ...
    // TB_GETMAXSIZE not always working
    SIZE Size;
    SendMessage(hwndToolBar, TB_GETMAXSIZE, 0,(LPARAM)&Size);
    rbBand.cx = Size.cx +4;
    //int NbButtons = (int)SendMessage(hwndToolBar,(UINT) TB_BUTTONCOUNT, 0,0);
    //if (NbButtons>0)
    //{
    //    SendMessage(hwndToolBar, TB_GETITEMRECT, NbButtons-1,(LPARAM)&RectButton);
    //    rbBand.cx = RectButton.right;
    //}
    
    // Show a chevron button if the band is smaller than cxIdeal
    rbBand.cxIdeal= Size.cx; // -6;
    ...
    // update size to take into account header (rebar name/icon)
    rbBand.fMask=RBBIM_HEADERSIZE|RBBIM_SIZE;
    SendMessage(this->hwndRebar,RB_GETBANDINFO,BandIndex,(LPARAM)&rbBand);
    if (rbBand.cxHeader)
    {
        rbBand.fMask=RBBIM_SIZE;
        rbBand.cx += rbBand.cxHeader;
        // warning don't update cxIdeal !
...
//           BOOL FixedSize : TRUE if band can't be resized (shouldn't be set in the same time as NewLine)
//           BOOL NoGripper : TRUE if band musn't have a gripper
//           int cxMinChild : 0 for min size equal to control width; !=0 if min size is different of control width
//     out : 
//     return : band index
//-----------------------------------------------------------------------------
UINT CRebar::AddBand(HWND hwnd,TCHAR* Name,BOOL NewLine,BOOL FixedSize,BOOL NoGripper,int cxMinChild)
{
    REBARBANDINFO rbBand={0};
    // Initialize structure members that both bands will share.
    rbBand.cbSize = sizeof(REBARBANDINFO);  // Required
    rbBand.fMask  = RBBIM_SIZE | RBBIM_CHILD | RBBIM_CHILDSIZE | RBBIM_ID | RBBIM_STYLE; // RBBIM_STYLE must be set due to RBBS_NOGRIPPER || RBBS_GRIPPERALWAYS
    ...
    rbBand.cxIdeal    = Rect.right-Rect.left;
    rbBand.cx         = rbBand.cxIdeal;
    rbBand.cxMinChild = cxMinChild?cxMinChild:rbBand.cxIdeal;
...
BOOL CRebar::SetBandMinChildWidth(UINT BandIndex , int MinChildWidth)
{
...
#pragma message(__FILE__ " FIX ME: SetBandMinChildWidth this code can modify the height of child control... :( ")
    if (!this->GetBandInfo(BandIndex , &BandInfo))
        return FALSE;

    BandInfo.cxMinChild = MinChildWidth;
    return this->SetBandInfo(BandIndex , &BandInfo);
}

BOOL CRebar::SetBandWidth(UINT BandIndex , int Width)
{
    REBARBANDINFO BandInfo;
    memset(&BandInfo,0,sizeof(BandInfo));
    BandInfo.cbSize = sizeof(BandInfo);
...
BOOL CRebar::GetBandInfo(UINT BandIndex , REBARBANDINFO* pBandInfo)
{
    return (SendMessage(this->hwndRebar,RB_SETBANDINFO,BandIndex,(LPARAM)pBandInfo) != 0);
}

BOOL CRebar::SetBandInfo(UINT BandIndex , REBARBANDINFO* pBandInfo)
{
    return (SendMessage(this->hwndRebar,RB_SETBANDINFO,BandIndex,(LPARAM)pBandInfo) != 0);
}
//-----------------------------------------------------------------------------
...
BOOL CRebar::OnNotify(WPARAM wParam, LPARAM lParam)
{
    ...
    case RBN_HEIGHTCHANGE:
        if (this->pRebarEvents)
            this->pRebarEvents->OnRebarHeightChange(this);
        break;
                    ...
                    if (this->GetChevronMenuInfos(ClickedItemId,&ChevronMenuInfos,&MenuOrCommandId,&pDropDownMenu,&IsMenuId))
                    {
                        if (IsMenuId)
                        {
                            if (pToolBar)
                            {
                                ...
                                pToolBarEvents = pToolBar->GetEventsHandler();
                                if (pToolBarEvents)
                                    pToolBarEvents->OnToolbarDropDownMenu(pToolBar,pDropDownMenu,MenuOrCommandId);
                            }
