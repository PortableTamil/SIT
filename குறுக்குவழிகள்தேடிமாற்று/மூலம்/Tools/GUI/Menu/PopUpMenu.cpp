...
{
    this->pPopUpMenuEvents = NULL;
    this->bUseThemingForVistaAndSeven = FALSE; // for 8 and 10 do like XP : use our own menu drawing

    OSVERSIONINFOEX VersionInfos;
    memset(&VersionInfos,0,sizeof(VersionInfos));
    VersionInfos.dwOSVersionInfoSize = sizeof(VersionInfos);
    ::GetVersionEx((OSVERSIONINFO*)&VersionInfos);
    if ( ( VersionInfos.dwMajorVersion==6 )
         && (VersionInfos.dwMinorVersion<2) // vista and seven only
       )
    {
        ...
        if( hmod )
        {
            ...
            this->bUseThemingForVistaAndSeven = (BOOL)pfIsThemeActive();
        }
...
// Object: set the events handler interface
// Parameters :
//     in  : 
//     out :
//     return : TRUE on success
//-----------------------------------------------------------------------------
BOOL CPopUpMenu::SetEventsHandler(IPopUpMenuEvents* pPopUpMenuEvents)
{
    this->pPopUpMenuEvents = pPopUpMenuEvents;
    return TRUE;
}
...
{
    return this->SetIcon(MenuID,(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED));
}
...
{
    // load icon
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,0,0,LR_DEFAULTCOLOR|LR_SHARED);
    return this->Add(Name,hIcon,this->GetItemCount());
}
...
{
    // load icon
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED);
...
//     return : Id of item menu (used to remove it),0 if failed
//-----------------------------------------------------------------------------
UINT CPopUpMenu::Add(TCHAR* Name,int IdIcon,HINSTANCE hInstance,int Width,int Height)
{
    // load icon
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED);
    return this->Add(Name,hIcon,this->GetItemCount());
...
{
    return this->AddEx(Name,hIcon,Index,this->GetNextMenuId());
}

UINT CPopUpMenu::AddEx(TCHAR* Name,UINT MenuId)
{
    return this->AddEx(Name,NULL,this->GetItemCount(),MenuId);
}
UINT CPopUpMenu::AddEx(TCHAR* Name,HICON hIcon,UINT MenuId)
{
    return this->AddEx(Name,hIcon,this->GetItemCount(),MenuId);
}
UINT CPopUpMenu::AddEx(TCHAR* Name,int IdIcon,HINSTANCE hInstance,int Width,int Height,UINT MenuId)
{
    // load icon
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED);
    return this->AddEx(Name,hIcon,this->GetItemCount(),MenuId);
}
UINT CPopUpMenu::AddEx(TCHAR* Name,int IdIcon,HINSTANCE hInstance,int Width,int Height,UINT Index,UINT MenuId)
{
    // load icon
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED);
    return this->AddEx(Name,hIcon,Index,MenuId);
}

UINT CPopUpMenu::AddEx(TCHAR* Name,HICON hIcon,UINT Index,UINT MenuId)
{
    MENUITEMINFO mii={0};
    mii.cbSize=sizeof(MENUITEMINFO);
...
//-----------------------------------------------------------------------------
// Name: RemoveAll
// Object: remove all items of menu
// Parameters :
//     in  : 
//     out : 
//     return : 
...
{
    this->RemoveAllEx(FALSE);
}

void CPopUpMenu::RemoveAllEx(BOOL ResetMenuId)
{
    int NbItems = this->GetItemCount();
    for (int Cnt = 0;Cnt<NbItems;Cnt++)
        ::RemoveMenu(this->hPopUpMenu,0,MF_BYPOSITION);
    if (ResetMenuId)
        this->CurrentMenuId = 0;
}
...
{
    return this->AddSubMenu(SubMenuName,SubMenu,IdIcon,hInstance,0,0,(UINT)-1);
}
...
//     return : Id of item menu (used to remove it),0 if failed
//-----------------------------------------------------------------------------
UINT CPopUpMenu::AddSubMenu(TCHAR* SubMenuName,CPopUpMenu* SubMenu,int IdIcon,HINSTANCE hInstance,int Width,int Height)
{
    return this->AddSubMenu(SubMenuName,SubMenu,IdIcon,hInstance,Width,Height,(UINT)-1);
}
...
{
    return this->AddSubMenu(SubMenuName,SubMenu,IdIcon,hInstance,0,0,Index);
}
...
//     return : Id of item menu (used to remove it),0 if failed
//-----------------------------------------------------------------------------
UINT CPopUpMenu::AddSubMenu(TCHAR* SubMenuName,CPopUpMenu* SubMenu,int IdIcon,HINSTANCE hInstance,int Width,int Height,UINT Index)
{
    HICON hIcon=(HICON)::LoadImage(hInstance, MAKEINTRESOURCE(IdIcon),IMAGE_ICON,Width,Height,LR_DEFAULTCOLOR|LR_SHARED);
...
{
    // ParentPopUpMenu must be set in constructor to assume that other method call will work gracefully
    // methods like GetNextMenuID, and so Add and AddSeparator require them
    // The following code has been disabled due to toolbar popup menu (SubMenu->ParentPopUpMenu == NULL)
    // in case of chevron, a top level popupmenu is added and the default toolbar popupmenu gets a temporary parent (the chevron popupmenu)
    // so the following code prevent chevron submenu to be set --> MUST be commented
    //    if (SubMenu->ParentPopUpMenu != this)
    //    {
    //#ifdef _DEBUG
...
void CPopUpMenu::SetMenuItemBitmapInfo(MENUITEMINFO* pMenuItem,HICON hIcon)
{
    if (hIcon)
    {
        ...
        if (this->bUseThemingForVistaAndSeven)
        {
            SIZE IconSize;
            ICONINFO IconInfo;
...
//-----------------------------------------------------------------------------
// Name: GetNameLength
// Object: retrieve menu name length from menu index
// Parameters :
//     in  : UINT Menu index : index
//     out : 
//     return : -1 if not found, else menu id
//-----------------------------------------------------------------------------
BOOL CPopUpMenu::GetNameLength(IN UINT MenuIndex,OUT SIZE_T* pLength)
{
    MENUITEMINFO MenuItemInfo={0};
    MenuItemInfo.cbSize=sizeof(MENUITEMINFO);
    MenuItemInfo.fMask=MIIM_STRING;
    *pLength = 0;
    BOOL bRetValue = ::GetMenuItemInfo(this->hPopUpMenu,MenuIndex,TRUE,&MenuItemInfo);
    *pLength = MenuItemInfo.cch+1;
    return bRetValue;
}

//-----------------------------------------------------------------------------
// Name: GetName
// Object: retrieve menu name from menu index
// Parameters :
//     in  : UINT Menu index : index
//     out : 
//     return : FALSE if not found, TRUE else
//-----------------------------------------------------------------------------
BOOL CPopUpMenu::GetName(IN UINT MenuIndex,IN OUT TCHAR* Name,IN SIZE_T NameMaxLength)
{
    MENUITEMINFO MenuItemInfo={0};
    MenuItemInfo.cbSize=sizeof(MENUITEMINFO);
    MenuItemInfo.fMask=MIIM_STRING;
    MenuItemInfo.dwTypeData = Name;
    MenuItemInfo.cch = (UINT)NameMaxLength;
    return ::GetMenuItemInfo(this->hPopUpMenu,MenuIndex,TRUE,&MenuItemInfo);
}
...
// Object: retrieve menu index from menu name (notice : in case multiple items with same name exists, first found index is return)
// Parameters :
//     in  : TCHAR* Name
//     out : 
//     return : -1 if not found, else menu index
//-----------------------------------------------------------------------------
int CPopUpMenu::GetIndexFromName(TCHAR* Name)
{
    int Cnt;
    SIZE_T Length=_tcslen(Name)+8; // +2: 1 for '\0', and 1 to assume string is exactly the same (not only same begin with one name shorter than the other)
    TCHAR* MenuName;
    MenuName = new TCHAR[Length];
    
    for (Cnt=0;Cnt<this->GetItemCount();Cnt++)
    {
        if (!this->GetName(Cnt,MenuName,Length))
            continue;
        if (_tcscmp(MenuName,Name)==0)
        {
            delete[] MenuName;
            return Cnt;
        }
...
UINT CPopUpMenu::Show(int x,int y, HWND hOwner,BOOL PositionRelativeToOwner,BOOL ShowUpper)
{
    ...
    // MSDN 
    // To display a context menu for a notification icon, the current window must be the foreground window before 
    // the application calls TrackPopupMenu or TrackPopupMenuEx. Otherwise, the menu will not disappear when the user 
    // clicks outside of the menu or the window that created the menu (if it is visible). 
...
// #ifdef _DEBUG
//    TCHAR ItemText[256];
//    this->GetText(lpdis->itemID,ItemText,_countof(ItemText));
...
// #endif
    ...
    if (this->bAllowIconsEffects)
    {
        ...
        if ( bSelected )
        {
            // draw background alpha grey icon (icon will be drawn upper current alpha grey)
...
BOOL CPopUpMenu::GetMenuFromPoint(POINT* pPoint,HMENU* phWndMenu,UINT* pMenuIndex)
{
    UINT MenuIndex;
    UINT MenuId;
    CPopUpMenu* pSubMenu;
    ...
    *pMenuIndex = 0;
    ...
    // find selected sub children
    for (MenuIndex=0;MenuIndex<(UINT)NbItems;MenuIndex++)
    {
        if (this->IsItemSubMenuFromIndex(MenuIndex))
        {
            // get only the highlighted sub menu
            if (!this->IsItemHighLightedSubMenuFromIndex(MenuIndex))
                continue;

            MenuId = this->GetID(MenuIndex);
            pSubMenu = this->GetSubMenu(MenuId);
            if (pSubMenu->GetMenuFromPoint(pPoint,phWndMenu,pMenuIndex))
                return TRUE;
        }
    }

    // if not found in child menus, look into current menu
    for (MenuIndex=0;MenuIndex<(UINT)NbItems;MenuIndex++)
    {
        if (!::GetMenuItemRect(NULL,this->GetControlHandle(),MenuIndex,&Rect))
            continue;
        if (::PtInRect(&Rect,*pPoint))
        {
            *phWndMenu = this->GetControlHandle();
...
{
    if (this->pPopUpMenuEvents)
        this->pPopUpMenuEvents->OnPopUpMenuMouseRightButtonUp(this,wParam,lParam);
}
...
{
    if (this->pPopUpMenuEvents)
        this->pPopUpMenuEvents->OnPopUpMenuMenuSelect(this,wParam,lParam);
}
...
LRESULT CALLBACK CPopUpMenu::SubClassWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,UINT_PTR uIdSubclass,DWORD_PTR dwRefData)
{
    ...
    // case WM_CONTEXTMENU: (WM_CONTEXTMENU not always sent WM_RBUTTONUP is better)
    case WM_RBUTTONUP:
        {
            // find top most parent
            CPopUpMenu* pRootPopUpMenu;
            pRootPopUpMenu=pPopUpMenu;
            while (pRootPopUpMenu->ParentPopUpMenu)
            {
                pRootPopUpMenu = pRootPopUpMenu->ParentPopUpMenu;
            }
           
            // get associated menu and menu index
            POINT pt;
            HMENU ClickedhWndMenu = NULL;
            UINT ClickedMenuIndex = 0;
            ::GetCursorPos(&pt);

            if (pRootPopUpMenu->GetMenuFromPoint(&pt,&ClickedhWndMenu,&ClickedMenuIndex))
                pPopUpMenu->OnMouseRightButtonUp(ClickedMenuIndex,(LPARAM)ClickedhWndMenu);
        }
        return 0; // event is treated cancel it and don't call DefSubclassProc

    // disabled to avoid double OnMouseRightButtonUp call
    //case WM_MENURBUTTONUP:
    //    // WM_MENURBUTTONUP is not send for sub menus when they are expanded
    //    // bug solved with WM_RBUTTONUP (WM_CONTEXTMENU not always sent)
