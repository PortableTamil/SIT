...
public:
    ...
    typedef BOOL (*tagLineCallBack)(TCHAR* FileName,TCHAR* Line,DWORD dwLineNumber,BOOL bLastLine,LPVOID UserParam);
    ...
    static BOOL ReadEx(TCHAR* FileName,TCHAR** ppszContent,ULONG64* pContentSize,BOOL* pbUnicode16File);
    static BOOL ParseLines(TCHAR* pszFileName,tagLineCallBack LineCallBack,LPVOID CallBackUserParam);
    static BOOL ParseLines(TCHAR* pszFileName,HANDLE hCancelEvent,tagLineCallBack LineCallBack,LPVOID CallBackUserParam, OUT BOOL* pbCanceled = NULL);
    FORCEINLINE static void CloseFile(HANDLE hFile){if (hFile!=INVALID_HANDLE_VALUE) ::CloseHandle(hFile);}
};
